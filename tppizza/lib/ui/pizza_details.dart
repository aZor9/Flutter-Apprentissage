...
class PizzaDetails extends StatefulWidget {
  ...
  PizzaDetails(this._pizza, this._cart, this.title, {Key? key}) : super(key: key);
...
class _PizzaDetailsState extends State<PizzaDetails> {
  ...
  _buildDropDownPates() {
    return DropdownButton<OptionItem>(
      isExpanded: true,
      value: Pizza.pates.firstWhere((p) => p.value == widget._pizza.pate, orElse: () => Pizza.pates[0]),
  ...
  _buildDropDownTailles() {
    return DropdownButton<OptionItem>(
      isExpanded: true,
      value: Pizza.tailles.firstWhere((p) => p.value == widget._pizza.taille, orElse: () => Pizza.tailles[0]),
  ...
  _buildDropDownSauces() {
    return DropdownButton<OptionItem>(
      isExpanded: true,
      value: Pizza.sauces.firstWhere((p) => p.value == widget._pizza.sauce, orElse: () => Pizza.sauces[0]),
